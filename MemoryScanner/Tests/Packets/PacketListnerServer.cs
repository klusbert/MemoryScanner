...
namespace MemoryScanner.Tests
{
    class PacketListnerServer
    {
        ...
        public PacketListnerServer(MemoryReader _memread)
        {
            memRead = _memread;
            ...
            while (running)
            {
                if (memRead.ReadByte(GotPacketAdr.ToInt32()) == 1)
                {
                    if (networkStream.Postion == 8)
                    {
                        ...
                        //lets try modify our incomming packets
        ...
        class NetWorkStream
        {
            ...
            public byte[] Data
            {
                get
                {
                    ...
                    packet = memRead.ReadBytes(streamPointer , (uint)Lenght );
                    return packet;
                }
            }
        }
